...
class Codec:
    ...
    def deserialize(self, data):
        ...
        if not a:
            return
        bt = [TreeNode(i) if i is not None else None for i in a]
        n = len(bt)
        offset = 0
        for i in range(n):
            if bt[i]:
                left = i * 2 + 1 - offset
                bt[i].left = bt[left] if left < n else None
                bt[i].right = bt[left + 1] if left + 1 < n else None
            else:
                offset += 2
...
ans = ser.serialize(ser.deserialize(data))
assert data == ans, ans
